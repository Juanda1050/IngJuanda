...
const playlists = [
  ...
  { "name": "Your All-Time Top Songs", "id": "37i9dQZF1CKMFBxUbxRIAg" },
...
async function startSync() {
  ...
  for (let i = 0; i < playlists.length; i++) {
    ...
    console.log(`[${i + 1}/${playlists.length}] Fetching playlist "${pl.name}" (${pl.id})...`);
    try {
      const data = await scrapePlaylistEmbed(pl.id);
      result.push(data);
      console.log(`  Successfully synced "${data.name}" with ${data.tracks.length} tracks.`);
    } catch (err) {

...
import { IOS_APPS, IOS_DOCK_APPS } from "./ios-layout.constants";
import { useIosLayout } from "./hooks/useIosLayout";

function IosSignalIcon() {
  return (
    <svg width="17" height="12" viewBox="0 0 512 470" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function IosWifiIcon() {
  return (
    <svg width="16" height="12" viewBox="0 0 1024 800" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function IosBatteryIcon({ level = 100, charging = false }: { level?: number; charging?: boolean }) {
  const fillColor = charging ? "#34c759" : level <= 20 ? "#ff3b30" : "currentColor";
  const innerX = 54.3;
  const innerY = 48.54;
  const innerW = 348.0;
  const innerH = 161.17;
  ...
  return (
    <svg width="17" height="12" viewBox="0 0 512 150" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className="select-none overflow-visible" aria-hidden="true">
      <defs>
        <clipPath id="battery-fill-clip">
          <rect x={innerX} y={innerY} width={innerW} height={innerH} rx="15" ry="15" />
        </clipPath>
      </defs>
      {fillW > 0 && (
        <rect x={innerX} y={innerY} width={fillW} height={innerH} rx="15" ry="15" fill={fillColor} clipPath="url(#battery-fill-clip)" />
      )}
      <path fillRule="nonzero" d="M472.06 38.53v29.62h27.84c3.36 0 6.38 1.37 8.55 3.55 2.19 2.19 3.55 5.25 3.55 8.55v97.76c0 3.32-1.39 6.37-3.56 8.54-2.17 2.16-5.22 3.56-8.54 3.56h-27.84v29.61c0 21.2-17.35 38.53-38.53 38.53h-395C17.33 258.25 0 240.93 0 219.72V38.53C0 17.35 17.37 0 38.53 0h395c21.22 0 38.53 17.31 38.53 38.53zm-69.88 10.01c8.57 0 15.59 7.13 15.59 15.59v129.99c0 8.46-7.13 15.59-15.59 15.59H69.88c-8.46 0-15.58-7.02-15.58-15.59V64.13c0-8.57 7.01-15.59 15.58-15.59h332.3zm44.42 168.41V41.3c0-8.69-7.15-15.84-15.83-15.84H41.3c-8.71 0-15.84 7.13-15.84 15.84v175.65c0 8.68 7.17 15.84 15.84 15.84h389.47c8.69 0 15.83-7.12 15.83-15.84z" />
      {charging && (
        <path d="M278 58 L230 140 h38 L244 198 L300 110 h-40 Z" fill="currentColor" opacity="0.9" />
      )}
    </svg>
  );
}

function IosAppContainer({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col h-full w-full bg-[#f2f2f7] dark:bg-black text-black dark:text-white font-sans overflow-hidden select-none pb-6">
      <div className="flex items-center justify-between px-4 h-[88px] bg-[#f2f2f7]/80 dark:bg-[#1c1c1e]/80 backdrop-blur-md border-b border-black/10 dark:border-white/10 z-10 pt-[44px] shrink-0">
        ...
        <span className="font-semibold text-base md:text-lg truncate max-w-[150px]">{title}</span>
        <div className="w-[60px]" />
      </div>
...
export function IosLayout() {
  ...
  const renderApp = () => {
    switch (activeApp) {
      case "dashboard":
        return <IosAppContainer title={t("dashboard.dockLabel")}><DashboardWindow /></IosAppContainer>;
      case "settings":
        return <IosSettings onClose={() => setActiveApp(null)} />;
      case "phone":
        return <IosAppContainer title="Phone"><PhoneWindow /></IosAppContainer>;
      case "notes":
        return <IosAppContainer title="Notes"><NotesWindow /></IosAppContainer>;
      case "calendar":
        return <IosAppContainer title="Calendar"><CalendarWindow /></IosAppContainer>;
      case "safari":
        return <IosAppContainer title="Safari"><SafariWindow /></IosAppContainer>;
      case "messages":
        return <IosAppContainer title="Messages"><MessagesWindow /></IosAppContainer>;
      case "mail":
        return <IosAppContainer title="Mail"><MailWindow /></IosAppContainer>;
      default:
        return null;
    }
  };
    ...
    <div className="fixed inset-0 bg-black overflow-hidden flex flex-col font-sans select-none antialiased text-white">
      ...
      <div
        ...
        className={cn(
          ...
          activeApp ? "text-black dark:text-white" : "text-white drop-shadow-md",
        )}
      >
        <span>
        ...
        <AnimatePresence>
          {activeApp ? (
            <motion.div
              ...
              style={{ x: edgeDragX, y: appY, scale: appScale, borderRadius: appBorderRadius, opacity: appOpacity }}
            ...
            >
              <div id="ios-app-grid" className="grid grid-cols-4 gap-x-4 gap-y-8 px-6">
                {IOS_APPS.map((app) => (
              ...
              <div id="ios-dock" className="mx-4 mb-8 rounded-[24px] bg-white/20 backdrop-blur-2xl border border-white/10 p-4 flex justify-between">
